...
if ($('#amlinechart5').length) {
    var estacion = $("#nodo").val();
    var escala = $("#escala").val();
    console.log('nodo ', $("#nodo").val());
    console.log('escala ', $("#escala").val());
    var settings = {
        "url": "http://localhost:3500/sensor/list",
        "method": "POST",
        "timeout": 0,
        "headers": {
            "Content-Type": "application/json"
        },
        "data": JSON.stringify({ "station": estacion, "variable": "Temperature", "escala": escala }),
    };
    var data = {};
...
function actualizaHistory(sensor) {
    ...
    var settings = {
        ...
        "data": JSON.stringify({ "station": estacion, "variable": sensor, "escala": escala }),
    };
    var data = {};
    $.ajax(settings).done(function (response) {
        console.log(response);
...
function cargarTabla(sensor) {
    $("#buscarValor").append('')
    var year = $("#year").val()
    var month = $("#month").val()
    var day = $("#day").val()
    var escala = $("#escalaNodo").val()
    ...
    var settings = {
        ...
        "data": JSON.stringify({ "date": fecha, "variable": sensor, "escala": escala }),
    };
    ...
    $.ajax(settings).done(function (response) {
        ...
        for (let nodo of data) {
            $("#buscarValor").html('')
            if (nodo.estacion_1 == null) {
                $("#buscarValor").append('<tr><td class="nodo">Nodo 1</td><td class="estado">Inactivo</td><td class="valor" id="valor1">No hay datos</td></tr>')
            } else {
                $("#buscarValor").append('<tr><td class="nodo">Nodo 1</td><td class="estado">Activo</td><td class="valor" id="valor1">' + nodo.estacion_1 + '</td></tr>')
            }
            if (nodo.estacion_2 == null) {
                $("#buscarValor").append('<tr><td class="nodo">Nodo 2</td><td class="estado">Inactivo</td><td class="valor" id="valor2">No hay datos</td></tr>')
            } else {
                $("#buscarValor").append('<tr><td class="nodo">Nodo 2</td><td class="estado">Activo</td><td class="valor" id="valor2">' + nodo.estacion_2 + '</td></tr>')
            }
...
function buscarValorActual(sensor) {
...
function buscarValorActualIni() {
    var estacion = $("#valorNodo").val();
    var sensor = $("#valorActual").val();
    console.log("Estacion: " + estacion)
    console.log("sensor inicialalalslaslasalsals: " + sensor)
    var station = "";
    if (estacion == "01") {
        station = "estacion_1"
    }
...
function animateGauges() {
    document.gauges.forEach(function (gauge) {
        ...
        }, gauge.animation.duration + 10));
    });
}
$(document).ready(buscarValorActual);
//------------echarts2
